...
// const detalhesVendaService = require('./detalhesVendaService'); // Não precisamos importar ele mesmo
...
class ValidacaoFreteService {
    ...
    async calcularFreteTotalPacote(itensDetalhados, colunaFrete, is_sulsudeste, is_fulfillment, sellerReputationLevelId, accessToken, shipping_id) {
        console.log(`calcularFreteTotalPacote: Iniciando cálculo de frete para ${itensDetalhados.length} itens no pacote`);
        ...
        for (const itemInfo of itensDetalhados) {
            if (!itemInfo.api) {
                console.warn(`calcularFreteTotalPacote: Item sem detalhes da API, não é possível calcular frete`);
                continue;
            }
        ...
        console.log(`calcularFreteTotalPacote: Detalhes do cálculo:`, JSON.stringify(detalhesCalculo));
    ...
    async obterDimensoesItem(itemId, accessToken, shippingId = null) {
        try {
            console.log(`obterDimensoesItem: Buscando dimensões para item ${itemId}`);
            
            // 1. Primeiro: tente obter dimensões do item via API se temos access token
            if (accessToken && shippingId) {
                try {
                    console.log(`obterDimensoesItem: Tentando obter via API para shipping ${shippingId}, item ${itemId}`);
                    ...
                    if (itensShipment && itensShipment.length > 0) {
                        ...
                        if (itemEncontrado && itemEncontrado.dimensions) {
                            ...
                            console.log(`obterDimensoesItem: Dimensões encontradas na API para ${itemId}:`, JSON.stringify(dimensions));
                    ...
                    console.warn(`obterDimensoesItem: Erro ao buscar da API: ${apiError.message}`);
                }
            }
            ...
            try {
                ...
                console.log(`obterDimensoesItem: Coluna item_id não disponível em DimensaoPacote`);
            }
            ...
            if (dimensoes) {
                console.log(`obterDimensoesItem: Dimensões encontradas no DB para ${itemId}:`, JSON.stringify(dimensoes));
                return {
                ...
                console.log(`obterDimensoesItem: Nenhuma dimensão encontrada para ${itemId}`);
                return null;
            }
        } catch (error) {
            console.error(`obterDimensoesItem: Erro ao buscar dimensões: ${error}`);
            return null;
        }
    }
    ...
    async salvarDimensoesItem(itemId, shippingId, dimensions) {
        try {
            ...
            if (existente) {
                ...
                console.log(`salvarDimensoesItem: Dimensões atualizadas para shipment ${shippingId}, item ${itemId}`);
            } else {
                // Criar novo registro
                ...
                console.log(`salvarDimensoesItem: Dimensões salvas para shipment ${shippingId}, item ${itemId}`);
            }
        } catch (error) {
            console.error(`salvarDimensoesItem: Erro ao salvar dimensões: ${error.message}`);
        }
    }

    // Busca os itens de um envio pelo shipment_id
    async obterItensPorShipment(shipmentId, accessToken) {
        console.log(`obterItensPorShipment: Obtendo itens para shipment ${shipmentId}`);
        if (!shipmentId || !accessToken) {
            console.error(`obterItensPorShipment: Parâmetros inválidos - shipmentId: ${shipmentId}`);
            return null;
        }
        ...
        try {
            ...
            if (!detalhesVendas || detalhesVendas.length === 0) {
                console.warn(`obterItensPorShipment: Não foram encontrados MLBs no banco para o shipment ${shipmentId}`);
            ...
            console.log(`obterItensPorShipment: MLB IDs encontrados no banco de dados: ${JSON.stringify(mlbIds)}, venda ID: ${vendaId}`);
                ...
                try {
                    ...
                    if (itemDetail) {
                        ...
                        try {
                            ...
                            console.warn(`obterItensPorShipment: Erro ao obter dimensões do item ${mlbId}: ${dimError.message}`);
                        }
                        ...
                        console.log(`obterItensPorShipment: Item ${mlbId} enriquecido - preço: ${enrichedItem.price}, condição: ${enrichedItem.condition}, quantidade: ${quantity}`);
                        enrichedItems.push(enrichedItem);
                    } else {
                        console.warn(`obterItensPorShipment: Não foi possível obter detalhes do item ${mlbId}`);
                    }
                } catch (itemError) {
                    console.error(`obterItensPorShipment: Erro ao processar item ${mlbId}: ${itemError.message}`);
                }
            }
            ...
            if (enrichedItems.length > 0) {
                ...
                console.warn(`obterItensPorShipment: Nenhum item pôde ser processado do banco de dados`);
                
                // Fallback para API original
                return await this.obterItensPorShipmentApi(shipmentId, accessToken);
            }
        } catch (error) {
            console.error(`obterItensPorShipment: Erro ao processar itens do banco: ${error.message}`);
    ...
    async obterItensPorShipmentApi(shipmentId, accessToken) {
        console.log(`obterItensPorShipmentApi: Fallback - Obtendo itens da API para shipment ${shipmentId}`);
                ...
                for (let i = 0; i < itemsData.length; i++) {
                    try {
                        ...
                        if (itemDetail) {
                            ...
                            enrichedItems.push(enrichedItem);
                            console.log(`obterItensPorShipmentApi: Item ${mlbId} enriquecido - preço: ${enrichedItem.price}, condição: ${enrichedItem.condition}`);
                        } else {
                            // Se não conseguiu enriquecer, adicionar item original
                            console.warn(`obterItensPorShipmentApi: Usando dados originais para item na posição ${i}`);
                            enrichedItems.push({
                                ...item,
                                id: mlbId || item.id
                            });
                        ...
                        console.error(`obterItensPorShipmentApi: Erro processando item: ${itemError.message}`);
    ...
    async calcularFreteIndividual(item, colunaFreteInicial, is_sulsudeste, is_fulfillment, sellerReputationLevelId, accessToken = null, shippingId = null) {
        console.log(`calcularFreteIndividual: Calculando frete para item individual:`, item.id);
        try {
            ...
            if (!item.price || !item.condition) {
                try {
                    ...
                    if (detalhesItem) {
                        console.log(`calcularFreteIndividual: Detalhes adicionais obtidos para item ${item.id} - preço: ${detalhesItem.price}, condição: ${detalhesItem.condition}`);
                        itemCompleto = {
                            ...item,
                    ...
                    console.warn(`calcularFreteIndividual: Erro ao obter detalhes completos do item ${item.id}: ${detailError.message}`);
            ...
            console.log(`calcularFreteIndividual: Dimensões iniciais da API para item ${itemCompleto.id}:`, JSON.stringify(dimensions));
            ...
            if (dimensionsEmpty) {
                console.log(`calcularFreteIndividual: Dimensões da API incompletas ou vazias para ${itemCompleto.id}, buscando via método otimizado`);
                const completeDimensions = await this.obterDimensoesItem(itemCompleto.id, accessToken, shippingId);
                if (completeDimensions) {
                    console.log(`calcularFreteIndividual: Dimensões completas encontradas para ${itemCompleto.id}:`, JSON.stringify(completeDimensions));
                    dimensions = completeDimensions;
                } else {
                    console.log(`calcularFreteIndividual: Nenhuma dimensão completa encontrada para ${itemCompleto.id}`);
                }
            }
            ...
            console.log(`calcularFreteIndividual: Dimensões finais utilizadas para ${itemCompleto.id}: ${comprimento}cm x ${largura}cm x ${altura}cm, peso ${peso_g}g`);
            console.log(`calcularFreteIndividual: Peso volumétrico calculado para ${itemCompleto.id}: ${pesoVolumetrico}kg`);
            
            // Usar o maior entre peso real e volumétrico
            const pesoCobravel = Math.max(peso_kg, pesoVolumetrico || 0);
            
            if (pesoCobravel <= 0) {
                console.warn(`calcularFreteIndividual: Peso cobrável inválido para item ${itemCompleto.id}`);
                return null;
            }
            ...
            if (itemCompleto.category_id) {
                ...
                console.log(`calcularFreteIndividual: Item ${itemCompleto.id} - Categoria ${itemCompleto.category_id} é especial: ${is_special_category}`);
            }
            
            // Garantir que o preço é um número e verificar se é item novo >= 79
            const itemPrice = Number(itemCompleto.price) || 0;
            const is_item_novo_79 = (itemCompleto.condition === 'new' && itemPrice >= 79);
            console.log(`calcularFreteIndividual: Item ${itemCompleto.id} - Preço: R$${itemPrice}, Condição: ${itemCompleto.condition}`);
            console.log(`calcularFreteIndividual: Item ${itemCompleto.id} - É novo>=79: ${is_item_novo_79}`);
    ...
    async obterQuantidadeItemPedido(vendaId, orderItemId, accessToken) {
        console.log(`obterQuantidadeItemPedido: Buscando quantidade para item ${orderItemId} na venda ${vendaId}`);
        if (!vendaId || !orderItemId || !accessToken) {
            console.error("obterQuantidadeItemPedido: IDs inválidos ou token ausente.");
            return null;
        }
        ...
        try {
            ...
            if (!response.ok) {
                const errorBody = await response.text();
                console.error(`obterQuantidadeItemPedido: Erro ${response.status} ao buscar pedido ${vendaId}: ${errorBody}`);
            ...
            if (orderData && Array.isArray(orderData.order_items)) {
                ...
                if (itemEncontrado && itemEncontrado.quantity !== undefined && !isNaN(Number(itemEncontrado.quantity))) {
                    ...
                    console.log(`obterQuantidadeItemPedido: Quantidade encontrada: ${quantity}`);
                    return quantity;
                } else {
                    console.warn(`obterQuantidadeItemPedido: Item ${orderItemId} não encontrado ou sem quantidade numérica na resposta do pedido ${vendaId}.`);
                    if(orderData.order_items.length > 0) { console.log("DEBUG - Estrutura item pedido:", JSON.stringify(orderData.order_items[0])); }
                }
            } else {
                console.warn(`obterQuantidadeItemPedido: Array 'order_items' não encontrado ou inválido na resposta do pedido ${vendaId}.`);
            }
        } catch (error) {
            console.error(`obterQuantidadeItemPedido: Erro na chamada API para pedido ${vendaId}:`, error);
            if (error.name === 'TokenError') throw error;
        }
        return null;
    }
    ...
    async validarFretePorPacote(shipping_id, accessToken) {
        ...
        try {
            const itensDoPacoteDB = await DetalhesVenda.findAll({
                ...
                attributes: ['venda_id', 'order_item_id', 'volume', 'weight', 'is_sulsudeste', 'is_fulfillment', 'total_amount', 'shipping_cost', 'is_pack'],
                raw: true
            });

            if (!itensDoPacoteDB || itensDoPacoteDB.length === 0) {
                 console.warn(`validarFretePorPacote: Nenhum item DB para ${shipping_id}.`);
                 ...
                 const is_sulsudeste_do_pacote = dadosComuns.is_sulsudeste; // Usar este
                 const is_fulfillment_do_pacote = dadosComuns.is_fulfillment; // Usar este
                 isPack = dadosComuns.is_pack === 1;

                 if (dadosComuns.shipping_cost && Number(dadosComuns.shipping_cost) > 0) {
                     compradorPagou = true;
                     console.log(`validarFretePorPacote: Comprador pagou R$ ${dadosComuns.shipping_cost} para ${shipping_id} (Venda: ${vendaIdDoPacote}).`);
                 }

                 if (chargeable_weight_kg === null || chargeable_weight_kg === undefined || isNaN(Number(chargeable_weight_kg))) {
                    console.warn(`validarFretePorPacote: Peso cobrável inválido para ${shipping_id} (Venda: ${vendaIdDoPacote}).`);
                    statusFinal = 'ERRO_SEM_PESO';
                     ...
                     if (freteCobradoAPI === null || freteCobradoAPI === undefined) {
                         console.warn(`validarFretePorPacote: Não obteve frete cobrado (API) para ${shipping_id} (Venda: ${vendaIdDoPacote}).`);
                         statusFinal = 'AVISO_SEM_FRETE_API';
                     } else if (Number(freteCobradoAPI) === 0 && compradorPagou) {
                         console.log(`validarFretePorPacote: Comprador pagou e senders_cost é 0 para ${shipping_id} (Venda: ${vendaIdDoPacote}).`);
                     ...
                     const itemDetailsPromises = itensDoPacoteDB.map(async (itemVenda) => {
                         ...
                         if (itemAPI) {
                             if (itemAPI.condition === 'new' && Number(itemAPI.price) >= 79) {
                                 temItemNovo79 = true;
                                 console.log(`validarFretePorPacote: Item ${itemVenda.order_item_id} é novo>=79 (preço: ${itemAPI.price})`);
                             }
                             ...
                             if (itemAPI.category_id) {
                                 ...
                                 if (isEspecial) {
                                     ...
                                     console.log(`validarFretePorPacote: Item ${itemVenda.order_item_id} tem categoria especial ${itemAPI.category_id}`);
                                 }
                             }
                         ...
                         if (!primeiroItemInfo) {
                             ...
                             console.warn(`validarFretePorPacote: Não foi possível obter detalhes do item ${itemVenda.order_item_id} via API`);
                         }
                     ...
                     console.log(`validarFretePorPacote: Total de ${totalQuantity} itens no pacote ${shipping_id} (Venda: ${vendaIdDoPacote}).`);
                     ...
                     console.log(`validarFretePorPacote: Dados - Sul/Sudeste: ${is_sulsudeste}, Fulfillment: ${is_fulfillment}, Pack: ${isPack}`);
                     console.log(`validarFretePorPacote: Dados - Item novo>=79: ${temItemNovo79}, Categoria especial: ${temCategoriaEspecial}`);
                     ...
                     colunaFreteUsada = await FreteService.selecionarColunaFrete(
                         dadoFreteItem,
                         itemProxy,
                         sellerReputationLevelId
                     );
                     ...
                     if (isPack && itensDetalhados.length > 1) {
                         console.log(`validarFretePorPacote: Pacote com múltiplos itens (${itensDetalhados.length}). Calculando frete individual para cada item...`);
                         ...
                         if (resultadoFretePacote && resultadoFretePacote.valor > 0) {
                             ...
                             statusPack = 'MULTIPLO';
                             console.log(`validarFretePorPacote: Frete total calculado para múltiplos itens: R$${freteCalculado}`);
                         } else {
                             console.warn(`validarFretePorPacote: Falha ao calcular frete para múltiplos itens. Voltando para método tradicional.`);
                     ...
                     if (!freteCalculado) {
                         ...
                         const faixaPeso = await FreteService.obterFaixaPeso(peso_kg, peso_volumetrico_kg);
                         freteCalculado = await FreteService.obterValorFrete(faixaPeso, colunaFreteUsada);
                         
                         console.log(`validarFretePorPacote: Peso: ${peso_kg}kg, Peso volumétrico: ${peso_volumetrico_kg}kg, Faixa: ${faixaPeso}`);
                         console.log(`validarFretePorPacote: Frete calculado (método tradicional): R$${freteCalculado}`);
                     }
                     
                     // Comparar com o frete cobrado pela API
                     if (freteCobradoAPI !== null) {
                         console.log(`validarFretePorPacote: Frete cobrado API: R$${freteCobradoAPI}`);
                         ...
                         if (Number(freteCobradoAPI) === 0 && compradorPagou) {
                             statusFinal = 'COMPRADOR_PAGOU';
                             console.log(`validarFretePorPacote: Comprador pagou frete, status: ${statusFinal}`);
                         ...
                         try {
                             const { ValidacaoPacote } = require('../../models');
                             ...
                             if (validacaoExistente) {
                                 await ValidacaoPacote.update(validacaoDados, {
                                     where: { shipping_id: shipping_id }
                                 });
                                 console.log(`validarFretePorPacote: Validação atualizada para ${shipping_id}`);
                             } else {
                                 await ValidacaoPacote.create(validacaoDados);
                                 console.log(`validarFretePorPacote: Validação criada para ${shipping_id}`);
                             }
                             
                             console.log(`validarFretePorPacote: Validação salva com sucesso para ${shipping_id}`);
                             return {
                                 shipping_id: shipping_id,
                                 status: statusFinal,
                                 frete_calculado: freteCalculado,
                             ...
                             console.error(`validarFretePorPacote: Erro ao salvar validação: ${dbError.message}`);
                             mensagemErro = `Erro ao salvar validação: ${dbError.message}`;
                         }
                     } else {
                         console.warn(`validarFretePorPacote: Sem frete cobrado pela API para comparar`);
        ...
        if (statusFinal.startsWith('ERRO') || mensagemErro) {
            try {
                ...
                if (validacaoExistente) {
                    await ValidacaoPacote.update(validacaoErro, {
                        where: { shipping_id: shipping_id }
                    });
                } else {
                    await ValidacaoPacote.create(validacaoErro);
                }
                
                console.log(`validarFretePorPacote: Validação de erro salva para ${shipping_id}`);
            } catch (finalError) {
