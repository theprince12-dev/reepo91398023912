...
class VendaService {
  async getAllSales(fromDate, toDate) {
    try {
         ...
         const url = `${config.api_base_url}/orders/search?seller=${sellerId}&order.date_created.from=${fromDate}&order.date_created.to=${toDate}`;
      ...
      for (const sale of sales) {
           const { id, date_created, buyer, status, total_amount, order_items } = sale;
           await Venda.upsert({
               order_id: id,
           ...
           const detalhesVenda = await detalhesVendaService.obterDetalhesVenda(id);
            await detalhesVendaService.salvarDetalhesVenda(id, detalhesVenda);
