...
class TaxaListagemService {
   async getListingPrices(price, category_id, listing_type_id) {
        try {
           ...
           const url = `${config.api_base_url}/sites/MLB/listing_prices?price=${price}&category_id=${category_id}`;
           ...
           let resultado_listagem = null
           if(data && data.listing_type_id === listing_type_id)
                resultado_listagem = data
            else if(Array.isArray(data))
             resultado_listagem = data.find(item => item.listing_type_id === listing_type_id);
